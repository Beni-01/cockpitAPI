...
ADD COLUMN IF NOT EXISTS validation_status ENUM('pending', 'valid', 'invalid', 'warning') DEFAULT 'pending' AFTER sync_hash,
ADD COLUMN IF NOT EXISTS validation_errors TEXT NULL AFTER validation_status,
ADD COLUMN IF NOT EXISTS validation_warnings TEXT NULL AFTER validation_errors,
ADD COLUMN IF NOT EXISTS validated_at TIMESTAMP NULL AFTER validation_warnings;
...
ALTER TABLE google_sheet_sync_log
ADD COLUMN IF NOT EXISTS queue_job_id VARCHAR(100) NULL AFTER id,
ADD COLUMN IF NOT EXISTS queue_priority ENUM('low', 'normal', 'high', 'critical') DEFAULT 'normal' AFTER queue_job_id,
ADD COLUMN IF NOT EXISTS queued_at TIMESTAMP NULL AFTER queue_priority,
ADD COLUMN IF NOT EXISTS processing_started_at TIMESTAMP NULL AFTER queued_at;

-- 5. Add indexes for queue queries
ALTER TABLE google_sheet_sync_log
ADD INDEX IF NOT EXISTS idx_queue_job_id (queue_job_id),
ADD INDEX IF NOT EXISTS idx_queue_priority (queue_priority);

-- 6. Create data_validation_rules table (optional - for custom validation rules)
...
-- 7. Create cache_metadata table (optional - for cache statistics)
...
ADD COLUMN IF NOT EXISTS auto_snapshot BOOLEAN DEFAULT TRUE AFTER use_polling,
ADD COLUMN IF NOT EXISTS snapshot_retention_days INT DEFAULT 30 AFTER auto_snapshot,
ADD COLUMN IF NOT EXISTS last_snapshot_at TIMESTAMP NULL AFTER snapshot_retention_days;
...
CREATE INDEX IF NOT EXISTS idx_budget_data_config_validated 
ON budget_data(config_id, validated_at);

CREATE INDEX IF NOT EXISTS idx_sync_log_status_started 
ON google_sheet_sync_log(status, started_at);
