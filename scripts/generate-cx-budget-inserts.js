...
// Read the CSV file - same source as FI but filtering for CX records
...
// Skip header row and process data rows - filter for CX records only
const dataRows = [];
for (let i = headerIndex + 2; i < lines.length; i++) {
  const line = lines[i].trim();
  // Process ALL records (FI and CX) for CX department
  ...
  // Handle both " _ " and " + " separators, and incomplete descriptions
  ...
  if (parts.length < 3) {
    ...
    parts = ['FINANCE', costCenter.replace('CX.', ''), costCenter.replace('CX.', ''), costCenter.replace('CX.', '')];
  }
...
console.log(`Found ${dataRows.length} CX data rows`);

// Generate SQL
let sql = `-- Generated from CSV: Fonarev_Budget_Direction Financière - Summary (1).csv
-- Date: ${new Date().toISOString().split('T')[0]}
-- Total CX (CAPEX) records: ${dataRows.length}
-- 
...
for (const row of dataRows) {
  const { costCenter, description, deptName, activityName, sousActivityName, tacheName, monthlyValues, totalBudget } = row;
...
// Write SQL file
const outputPath = path.join(__dirname, '../DB_Scripts/insert-cx-budget-from-csv.sql');
