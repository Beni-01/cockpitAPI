...
(async ()=>{
  ...
  try {
    ...
    if (!tacheId) { console.log('No tache id available; aborting budget insert.'); return; }
    ...
    if (APPLY) {
      ...
      console.log('Inserted budget id=', res.insertId);
    } else {
      console.log('Run with --apply to insert the budget row into the DB');
    }
