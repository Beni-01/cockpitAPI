...
// Find the header row (contains "Cost Center")
let headerIndex = -1;
...
for (let i = headerIndex + 2; i < lines.length; i++) {
  ...
  // Process ALL records (FI and CX) for FI department
  if (!line || (!line.startsWith('FI.') && !line.startsWith('CX.'))) continue;
  
  // Split by comma but handle numbers with spaces (like "152 600")
  ...
  // Extract monthly values (handle spaces in numbers)
  const monthlyValues = [];
  for (let m = 0; m < 12; m++) {
...
SET @dept_id = (SELECT id FROM department WHERE code = 'FI' LIMIT 1);
...
// Write SQL file
const outputPath = path.join(__dirname, '../DB_Scripts/insert-fi-budget-from-csv.sql');
